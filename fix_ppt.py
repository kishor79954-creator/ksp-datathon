...
for i, slide in enumerate(prs.slides):
    if i in slide_contents:
        ...
        # Find the text box shape on the slide (usually the only text box, or the one with text)
        text_shape = None
        ...
        if text_shape:
            # Clear all existing text and formatting
            text_frame = text_shape.text_frame
            text_frame.clear()
            
            # Slide 0 has no "title" in the template, it's just a raw text box
            if i == 0:
                ...
                p_body.text = content["body"]
                p_body.font.size = Pt(20)
                p_body.font.bold = False

prs.save(r"C:\Users\Kishore\Downloads\KSP Datathon 2026 _ Prototype Submission Template_Filled_v2.pptx")
print("Saved filled PPTX to _Filled_v2.pptx with corrected layout.")
